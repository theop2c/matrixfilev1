...
export function MatrixList() {
  ...
  useEffect(() => {
    ...
    async function loadMatrices() {
      ...
      try {
        logger.debug('Fetching matrices', { userId: user.id });
        await fetchMatrices(user.id);
        logger.debug('Matrices fetched successfully', { count: matrices.length });
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to fetch matrices';
    ...
    return () => {
      logger.debug('MatrixList unmounted');
    };
  }, [user, fetchMatrices]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      logger.debug('Deleting matrix', { matrixId: matrixToDelete.id });
      await deleteMatrix(matrixToDelete.id);
      logger.debug('Matrix deleted successfully');
